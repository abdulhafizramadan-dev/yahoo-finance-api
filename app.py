...
def api_sectors_summary(region: str = "id"):
    """
    Get today's sector performance summary for a given market region.

    Parameters:
    - region: Market region (default: "id" for Indonesia, supports any Yahoo Finance region code)

    Returns: {"sectors": [...], "count": 11, "region": "id"}

    Each sector contains:
    - name: Sector display name
    - key: Sector key (e.g., "financial-services")
    - change_percent: Average % change across all stocks in the sector today
    - stock_count: Number of stocks sampled in the sector
